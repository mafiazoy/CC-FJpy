...
cpdef fj_full(
    ...
    FJ(U_f1, r1, f1, out1, c1, nc, nr, nf, type,nThread)
...
cpdef fH_full(
    ...
    FH(U_f1, r1, f1, out1, c1, nc, nr, nf, type,nThread)
...
def fj(uf,r,c,f,fstride=1,itype=1,nThread=20):
    ...
    fj_full(uf,r,f,out,c,nc,nr,nf,itype,nThread)
...
def fh(uf,r,c,f,fstride=1,itype=1,nThread=20):
    ...
    if uf.dtype !=np.float32:
        uf = np.array(uf,dtype=np.float32) # Make sure the data type
    out = np.zeros(nf*nc,dtype=np.float32)
    fH_full(uf,r,f,out,c,nc,nr,nf,itype,nThread)
    out = out.reshape([nc,nf])
    return out
